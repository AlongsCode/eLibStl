...
EXTERN_C void fn_eStl_GetSM3(PMDATA_INF pRetData, INT nArgCount, PMDATA_INF pArgInf)
{
	auto data = elibstl::args_to_ebin(pArgInf, 0);
	unsigned char output[32]{};
	sm3(data->m_data, data->m_size, output);
	std::string Hex;
	
	for (size_t i = 0; i < 32; i++)
	{
		char pText[16]{};
		sprintf(pText, "%X", output[i]);
		std::string tmp = CharLowerA(pText);
		Hex = Hex + (tmp.size() == 1 ?  "0" + tmp : tmp);
	}
	pRetData->m_pText = elibstl::clone_text(Hex);
}
...
FucInfo Fn_eStl_GetSM3 = { {
		/*ccname*/  ("取数据SM3摘要"),
		/*egname*/  ("GetSM3Str"),
		/*explain*/ ("返回一段字节集数据的SM3数据摘要编码文本。不同数据的MD5码都不一样，因此本命令可以用作保证重要数据不会被篡改。"),
		/*category*/17,
		/*state*/   NULL,
