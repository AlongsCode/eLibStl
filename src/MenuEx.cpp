...
{
	{"菜单句柄", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE}
};
...
{
	{"对象析构时是否销毁菜单", "", 0, 0, SDT_BOOL, 0,ArgMark::AS_NONE}
};
...
{
	{"标题", "", 0, 0, SDT_BIN, 0, ArgMark::AS_DEFAULT_VALUE_IS_EMPTY},
	{"插入位置", "以下两种情况下将插入到菜单末尾：1)该参数为空；2)参数三为真且该参数为负数", 0, 0, SDT_INT, 0, ArgMark::AS_DEFAULT_VALUE_IS_EMPTY},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"状态", "#菜单项状态_ 常量，使用 位或() 命令组合状态", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"ID", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"子菜单句柄", "", 0, 0, SDT_INT, 0, ArgMark::AS_DEFAULT_VALUE_IS_EMPTY},
	{"自定义数值", "", 0, 0, SDT_INT, 0, ArgMark::AS_DEFAULT_VALUE_IS_EMPTY},
	{"位图句柄", "指定一个位图句柄或图片字节集或 #菜单预设位图_ 常量。"
			"注意：1)有大量图片需要被复用时不应传递字节集，这会导致性能问题；"
			"2)若本参数传递了字节集，则菜单Ex对象的生命周期必须持续到菜单被销毁之后，因为本对象内部维护位图句柄。", 0, 0, _SDT_ALL, 0, ArgMark::AS_DEFAULT_VALUE_IS_EMPTY},
	{"是否为分隔线", "", 0, 0, SDT_BOOL, FALSE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"换列选项", "0 - 常规  1 - 另起一列  2 - 另起一列并使用竖线分隔", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
};
...
{
	{"是否依附", "", 0, 0, SDT_BOOL, FALSE, ArgMark::AS_HAS_DEFAULT_VALUE}
};
...
static ARG_INFO s_Args_SetCheck[] =
{
	{"位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"是否选中", "", 0, 0, SDT_BOOL, 0, ArgMark::AS_NONE},
};
...
{
	{"位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"互斥范围起始位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"互斥范围结束位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"是否选中", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
};
...
{
	{"位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_DEFAULT_VALUE_IS_EMPTY},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
};
...
static ARG_INFO s_Args_SetDefault[] =
{
	{"位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"是否默认", "", 0, 0, SDT_BOOL, 0,ArgMark::AS_NONE},
};
...
static ARG_INFO s_Args_SetDisable[] =
{
	...
	{"是否灰色", "若此参数为假，则仅禁用项目，而不改变项目的外观，仅当 是否禁止 参数为真时有效", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
};
...
static ARG_INFO s_Args_SetHilite[] =
{
	{"位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"窗口句柄", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"是否高亮", "", 0, 0, SDT_BOOL, 0, ArgMark::AS_NONE},
};
...
{
	{"横向位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"纵向位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"窗口句柄", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"水平对齐", "0 - 左  1 - 居中  2 - 右", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"垂直对齐", "0 - 上  1 - 居中  2 - 下", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"通知选项", "0 - 常规  1 - 不通知父窗口  2 - 返回选中项目ID  3 - 不通知父窗口且返回选中项目ID", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"动画选项", "0 - 常规  -1 - 禁止动画  1 - 从上到下  2 - 从下到上  4 - 从左到右  8 - 从右到左，使用 位或() 命令做合理组合", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"鼠标按键选项", "0 - 左右键可选择  1 - 左键可选择", 0, 0, SDT_INT, 0, ArgMark::AS_HAS_DEFAULT_VALUE},
};
...
static ARG_INFO s_Args_SetCaption[] =
{
	{"位置", "", 0, 0, SDT_INT, 0, ArgMark::AS_NONE},
	{"位置是否为索引", "", 0, 0, SDT_BOOL, TRUE, ArgMark::AS_HAS_DEFAULT_VALUE},
	{"标题", "", 0, 0, SDT_BIN, 0, ArgMark::AS_NONE},
};
...
static ARG_INFO s_Args_SetEventReceiver[] =
{
	...
	{"事件接收子程序", "只能接收整数型和子程序指针型，子程序返回值为整数型，有一个整数型参数，"
			"该参数为菜单ID，返回0跳过默认处理，返回1执行默认处理", 0, 0, _SDT_ALL, 0, ArgMark::AS_NONE}
};
