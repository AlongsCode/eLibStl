...
{
	/*000*/ {DATA_TYPE::SDT_INT, 0, "左声道", "AUDIO_CHANNEL_LEFT", NULL, LES_HAS_DEFAULT_VALUE, (INT)1},
	/*001*/ {DATA_TYPE::SDT_INT, 0, "右声道", "AUDIO_CHANNEL_RIGHT", NULL, LES_HAS_DEFAULT_VALUE, (INT)2},
	/*002*/ {DATA_TYPE::SDT_INT, 0, "立体声", "AUDIO_CHANNEL_STEREO", NULL, LES_HAS_DEFAULT_VALUE, (INT)3},
	/*003*/ {DATA_TYPE::SDT_INT, 0, "均衡", "AUDIO_CHANNEL_AVERAGE", NULL, LES_HAS_DEFAULT_VALUE, (INT)4},
};
...
static ARG_INFO m_media_arg[] =
{
	...
	{"窗口句柄", "要设置的窗口句柄", 0, 0, DATA_TYPE::SDT_INT, 0, NULL},
	{"播放位置", "该参数为播放位置，以毫秒为单位。默认为-1，表示从当前处播放", 0, 0, DATA_TYPE::SDT_INT, -1, AS_HAS_DEFAULT_VALUE},
	{"左声道音量", "范围为0-100，0为静音，100为最大音量。如果本参数被省略或超出范围，则不修改此声道音量。", 0, 0, DATA_TYPE::SDT_SHORT, 0, AS_DEFAULT_VALUE_IS_EMPTY},
	{"右声道音量", "范围为0-100，0为静音，100为最大音量。如果本参数被省略或超出范围，则不修改此声道音量。", 0, 0, DATA_TYPE::SDT_SHORT, 0, AS_DEFAULT_VALUE_IS_EMPTY},
	{"左声道音量", "提供参数数据时只能提供变量。范围为0-100，0为静音，100为最大音量。如果本参数被省略，则不返回此声道音量。", 0, 0, DATA_TYPE::SDT_SHORT, 0, AS_RECEIVE_VAR},
	{"右声道音量", "提供参数数据时只能提供变量。范围为0-100，0为静音，100为最大音量。如果本参数被省略，则不返回此声道音量。", 0, 0, DATA_TYPE::SDT_SHORT, 0, AS_RECEIVE_VAR},
	{"声道类型", "声道类型常量1.左声道,2.右声道3.立体声4.均衡", 0, 0, DATA_TYPE::SDT_INT, 0, 0},
	{"媒体音量", "需设置媒体音量的大小", 0, 0, DATA_TYPE::SDT_INT, 0, 0},
	{"播放速度", "并非所有媒体都支持速度调整,1到2000之间的整数。原始速度为1000。2000为原始速度的2倍，500为原始速度的一半。警告:过慢的速度将导致媒体看起来已经停止了", 0, 0, DATA_TYPE::SDT_INT, 0, 0},
	{"位置", "", 0, 0, DATA_TYPE::SDT_INT, 0, 0},
	{"是否立即播放", "默认为假", 0, 0, DATA_TYPE::SDT_BOOL, 0, AS_DEFAULT_VALUE_IS_EMPTY},
...
FucInfo Media_IsVideo = { {
		"是否视频", "IsVideo", "判断当前播放的内容是否是视频。如果是返回真，否则返回假", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0, 0
...
FucInfo Media_GetHwnd = { {
		"取句柄", "GetHwnd", "获取当前视频播放窗口的Windows 窗口句柄", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0, 0
...
FucInfo Media_SetHwnd = { {
		"置句柄", "SetHwnd", "设置当前视频播放窗口的Windows 窗口句柄，成功返回真，失败返回假", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 1, m_media_arg + 2} ,
		Fn_media_SetHwnd ,
		"Fn_media_SetHwnd" };
...
FucInfo Media_GetMode = { {
		 "取状态", "GetMode", "获取当前的播放状态，为以下常量值之一：0、#未知状态；1、#播放状态；2、#停止状态；3、#暂停状态", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_GetMode ,
		"Fn_media_GetMode" };
...
FucInfo Media_GetLength = { {
		"取长度", "GetLength", "获取当前媒体文件的长度", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_GetLength ,
		"Fn_media_GetLength" };
...
FucInfo Media_GetTotalSec = { {
		"取总时间", "GetTotalSec", "以秒为单位获取总播放时间", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0,0},
...
FucInfo Media_GetFrameRate = { {
		 "取比率", "GetFrameRate", "获取每一帧的比率", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0, 0},
...
FucInfo Media_Play = { {
		"播放", "Play", "从指定位置开始播放。成功返回真，失败返回假,播放窗口为打开或者置句柄的指定的窗口", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 1, m_media_arg + 3}
		,Fn_media_Play,
		"Fn_media_Play" };
...
FucInfo Media_Pause = { {
		"暂停", "Pause", "暂停播放。成功返回真，失败返回假", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_Pause,
		"Fn_media_Pause" };
...
FucInfo Media_Stop = { {
		"停止","Stop", "停止播放。成功返回真，失败返回假", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_Stop,
		"Fn_media_Stop" };
...
FucInfo Media_GetMCIAlias = { {
		"取别名", "GetMCIAlias", "获取当前对象使用的MCI别名", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BIN, 0, LVL_SIMPLE, 0, 0, 0, 0},
...
FucInfo Media_Close = { {
		"关闭", "Close", "关闭当前播放的媒体。关闭后无法再对其进行任何操作,成功返回真，失败返回假。", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_Close,
		"Fn_media_Close" };
...
FucInfo Media_SetVolume = { {
		"置音量", "SetVolume", " 设置当前播放媒体的音量，不影响系统音量。左右声道中只要有一个设置失败，就会返回假。", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 2, m_media_arg + 4},
		Fn_media_SetVolume,
		"Fn_media_SetVolume" };
...
FucInfo Media_GetVolume = { {
		"取音量", "GetVolume", "获取当前播放音量（非系统音量）。左右声道中只要有一个获取失败，就会返回假。", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 2, m_media_arg + 6},
		Fn_media_GetVolume,
		"Fn_media_GetVolume" };
...
FucInfo Media_continue = { {
		"继续", "continue", "继续从暂停位置播放", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_continue,
		"Fn_media_continue" };
...
FucInfo Media_GetChannel = { {
		"取媒体声道", "GetChannel", "返回常量中的声道类型。失败返回0", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_GetChannel,
		"Fn_media_GetChannel" };
...
FucInfo Media_SetChannel = { {
		"置媒体声道", "SetChannel", "设置媒体声道类型", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 1, m_media_arg + 8},
		Fn_media_SetChannel,
		"Fn_media_SetChannel" };
...
FucInfo Media_GetFrames = { {
		"取帧数", "GetFrames", "取媒体文件的帧数", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0, 0},
		Fn_media_GetFrames,
		"Fn_media_GetFrames" };
...
FucInfo Media_GetMediaVolume = { {
		"取媒体音量", "GetMediaVolume", "取打开的媒体文件的音量", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0, 0},
...
FucInfo Media_SetMediaVolume = { {
		"置媒体音量", "SetMediaVolume", "执行后的实际音量可能最多±2的误差，这里设置的是系统的音量值", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 1,m_media_arg + 9},
...
FucInfo Media_SetPlaySpeed = { {
		"置播放速度", "SetPlaySpeed", "设置当前媒体的播放速度。", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 1,m_media_arg + 10},
...
FucInfo Media_GetPlaySpeed = { {
		"取播放速度", "GetPlaySpeed", "获取当前媒体的播放速度。", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_INT, 0, LVL_SIMPLE, 0, 0, 0,0},
...
FucInfo Media_SetPos = { {
		"置位置", "SetPos", "设置媒体当前播放的位置", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 2,m_media_arg + 11},
		Fn_media_SetPos,
		"Fn_media_SetPos" };
...
FucInfo Media_SetFrame = { {
		"跳到指定帧", "SetFrame", "跳到指定帧", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 2,m_media_arg + 11},
		Fn_media_SetFrame,
		"Fn_media_SetFrame" };
...
FucInfo Media_IsOpen = { {
		"是否已打开", "IsOpen", "判断是否已打开", -1, _CMD_OS(__OS_WIN), DATA_TYPE::SDT_BOOL, 0, LVL_SIMPLE, 0, 0, 0,0},
		Fn_media_IsOpen,
		"Fn_media_IsOpen" };
